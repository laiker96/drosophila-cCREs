...
\date{Repository documentation revision: 2026-08-04}
...
The default links branch requires no contact NGS data: it assigns every
enhancer-like catalog element to the nearest annotated TSS, keeps all exact
ties in one enhancer row, and annotates enhancer and promoter activity across
all contexts in columns.  An optional branch, enabled explicitly, normalizes
...
Thus nearest distance, observed contacts, and modeled distance evidence remain
separate products.
...
H3K27ac, a high-component posterior probability, TSS proximity, blacklist
  ...
  \item use the nearest annotated TSS as the default candidate gene, annotate
  that promoter's activity across contexts, and optionally add matched contact
  evidence plus an explicitly labeled power-law distance prior where maps are
  absent, without turning any ranking into a causal target assignment.
\end{enumerate}
...
contact-independent candidate-gene linking, optional contact-map normalization,
...
$\rightarrow$ nearest-TSS links [$\rightarrow$ optional contact links] $\rightarrow$
...
links & Checksummed nearest-TSS link checkpoint, with optional contact products & \stage{links} or \stage{report} \\
...
\section{Candidate gene branches}

\subsection{Default contact-independent nearest-TSS table}

The completed regulatory catalog feeds two independent link branches
(Figure~\ref{fig:catalog-link-branches}).  The default branch requires the
reference GTF and chromosome sizes but no contact NGS data.  It writes
\filep{activity/links/nearest\_tss/enhancer\_nearest\_tss\_candidates\_wide.tsv.gz}
with exactly one row for every catalog element classified
\texttt{proximal\_enhancer\_like} or \texttt{distal\_enhancer\_like}.  The
enhancer is not filtered by its H3K27ac posterior in this atlas-wide table.
...
\]
where \(t_p\) is an exact annotated TSS on any GTF chromosome represented in
the reference chromosome-sizes file.  Thus every exact distance tie is
retained.  Multiple tied promoter, gene, TSS, strand, and transcript values
are stored in aligned fields in the same enhancer row; ties never duplicate
the enhancer row.  This selection uses annotated TSS distance, not promoter
activity, and has no maximum-distance cutoff.

Shared columns contain master-DHS coordinates and class, minimum distance,
tie count, promoter IDs, gene IDs/names, TSS positions/strands, and transcript
IDs.  For each configured context, a prefixed column block contains the
enhancer's membership, ATAC signal, H3K27ac posterior, activity state, and
combined activity.  The same block reports whether any promoter in
\(\mathcal{N}_i\) is active, the active tied promoter IDs, their supporting
master-DHS IDs, and maximum promoter ATAC, H3K27ac-posterior, combined-activity,
and activity-score values.  Summary columns list all contexts in which at
least one tied nearest promoter is active.

Promoter activity uses the same explicit rule throughout the two branches:
a context-member master-DHS summit must lie within inclusive \(\pm500\) bp of
the exact TSS, and the maximum H3K27ac high-component posterior over those
supporting DHSs must be at least 0.5.  Activity annotates the already selected
nearest promoter; it does not replace it with a more distant active promoter.
The sibling promoter table and JSON metrics record the GTF/reference hashes,
thresholds, tie counts, per-context active-nearest-promoter counts, and output
hash.
...
\begin{tikzpicture}[
  ...
  source/.style={draw=pipelineblue,rounded corners,thick,fill=lightgold,
    text width=4.9cm,minimum height=1.05cm,inner sep=6pt},
  branch/.style={draw=pipelineblue,rounded corners,thick,
    text width=5.4cm,minimum height=1.55cm,inner sep=6pt},
  product/.style={draw=gray!65,rounded corners,text width=5.4cm,
    minimum height=1.35cm,inner sep=6pt},
  flow/.style={->,thick,color=pipelineblue}
]
  \node[source] (catalog) {Completed context-resolved catalog\\
    master-DHS coordinates, TSS class, and activity};
  \node[branch,fill=lightgreen,below left=16mm and 11mm of catalog] (nearest) {
    Default branch: no contact data\\nearest annotated TSS on the chromosome\\
    all exact ties, promoter activity by context};
  \node[branch,fill=lightblue,below right=16mm and 11mm of catalog] (contact) {
    Optional \texttt{--with-contacts} branch\\seven observed Micro-C/Hi-C
    contexts\\two labeled atlas power-law contexts};
  \node[product,below=9mm of nearest] (nearestout) {
    One enhancer per row\\shared nearest-gene fields +
    context-prefixed enhancer/promoter activity};
  \node[product,below=9mm of contact] (contactout) {
    Per-context nodes, promoter edges, gene ranks, focused observed-contact
    and power-law tables};
  \draw[flow] (catalog) -- (nearest);
  \draw[flow] (catalog) -- (contact);
  \draw[flow] (nearest) -- (nearestout);
  \draw[flow] (contact) -- (contactout);
\end{tikzpicture}
}
\caption{The catalog is the common immutable boundary.  The nearest-TSS
branch is the default and has no contact dependency.  Contact normalization
and contact/power-law candidate tables form a separate explicit opt-in branch.}
\label{fig:catalog-link-branches}
\end{figure}
...
recorded by the source catalog.  The default nearest-TSS branch works for a
supported catalog/reference without requiring the canonical dm6 context set.
Only the optional contact branch requires all nine canonical dm6 contexts.
The link rules stream the long table and select only rows belonging to the
requested context.  Quantification, mixture fitting, catalog construction, BigWig
...
\subsection{Optional contact mapping and source acquisition}

The contact branch is enabled only by \texttt{--with-contacts} while creating
a new master/quantification-to-links/report configuration or a new
catalog-bundle-to-links configuration.  It additionally requires the complete
canonical nine-context dm6 atlas.  Its reviewed source
table is \filep{resources/atlas_contact_sources.tsv}, with structure documented
by \filep{schemas/contact-sources.schema.yaml}.  Restricting this explicit
...
windows are 1,000 bp (plus/minus 500 bp) centered on the zero-based TSS and clipped at chromosome
boundaries.
...
\begin{tikzpicture}[
  ...
  object/.style={draw=pipelineblue,rounded corners,thick,minimum height=1.05cm,
    text width=3.05cm,inner sep=6pt},
  relation/.style={->,thick,color=pipelineblue},
  evidence/.style={draw=gray!65,rounded corners,minimum height=0.95cm,
    text width=3.2cm,inner sep=6pt}
]
  \node[object,fill=lightgold] (enhancer) {Master enhancer DHS\\
    \texttt{DHS\ldots}\\original interval + summit};
  \node[object,fill=lightblue,right=25mm of enhancer] (promoter) {
    Annotated promoter node\\\texttt{DM6PROM\ldots}\\1,000-bp window ($\pm500$) + exact TSS};
  \node[object,fill=lightgreen,right=18mm of promoter] (gene) {
    GTF gene and transcripts\\\texttt{gene\_id}, \texttt{gene\_name}\\alternative TSSs stay distinct};
  \node[object,fill=lightgold,below=12mm of promoter] (support) {
    Promoter-supporting master DHS\\summit within $\pm500$ bp + context activity};
  \node[evidence,fill=lightpurple,below=12mm of enhancer] (contact) {
    Pair evidence\\observed matrix pixel or\\distance expectation $E(d)$};

  \draw[relation] (support) -- node[right,font=\scriptsize] {
    $|\mathrm{summit}-\mathrm{TSS}|\leq500$ bp $\rightarrow P_p$} (promoter);
  \draw[relation] (enhancer) -- node[above,font=\scriptsize] {
    summit-to-TSS distance $d$} (promoter);
  \draw[relation] (promoter) -- node[above,font=\scriptsize] {GTF annotation} (gene);
  \coordinate (pairmid) at ($(enhancer.east)!0.5!(promoter.west)$);
  \draw[relation,dashed] (contact.north east) -- (pairmid);
\end{tikzpicture}
\caption{The master DHS and promoter registries are different data types.
Master-element boundaries are never replaced by the fixed promoter window.
Summit-to-TSS distance selects promoter-supporting DHSs and also anchors link
distance and contact evidence.}
\label{fig:contact-data-types}
\end{figure}

The class boundary and promoter-node window now express the same biological
choice: a summit at most 500 bp from its nearest TSS is
\texttt{promoter\_associated}, whereas enhancer-like labels begin beyond 500
bp.  The class is stored in the regulatory catalog.  A links-only import of an
older catalog therefore preserves its historical labels; applying the new
boundary requires rebuilding the catalog under a new run namespace.  Promoter
nodes themselves are regenerated from the GTF using the current window.  The
support calculation uses the inclusive condition
$|\mathrm{summit}-\mathrm{TSS}|\leq500$ bp directly; it is not an interval
overlap between the DHS boundaries and the promoter BED interval.

For a given context, all context-member master DHSs whose summits are at most
500 bp from a promoter TSS contribute to its annotation.  The node records
their IDs, maximum ATAC signal, maximum H3K27ac high-component posterior, and
maximum geometric activity
$A_j=\sqrt{\mathrm{ATAC}_j\times\mathrm{H3K27ac}_j}$.  It is marked active
when at least one such DHS exists and the maximum posterior is at least 0.5.
The continuous promoter activity score is
...
posterior from two different supporting sites from being combined.  Using
...
Within the optional contact branch, independently of its contact evidence,
every context also receives the narrower supplementary table
\filep{<context>.nearest_active_promoter_gene_candidates.tsv.gz}.  It uses the
same 0.5 enhancer-posterior threshold, then chooses the closest active
annotated promoter TSS on the same chromosome with no distance cutoff.  All
exact distance ties are retained as separate promoter rows.  A promoter is
active by the established summit-distance definition above.  Each output row
therefore lists all active master DHSs supporting the 1,000-bp target promoter
window and separately lists the subset whose catalog class is
\texttt{promoter\_associated}.  With a catalog built under the current rules,
all supporting DHSs necessarily have that class because the activity and class
definitions use the same inclusive 500-bp threshold.  The subset remains an
audit field for imported catalogs that retain historical labels.  The table has
\texttt{evidence\_type=nearest\_active\_promoter\_tss}; it uses neither a
matrix pixel nor $E(d)$.  It differs from the default wide table: it filters
enhancers at posterior 0.5, chooses among promoters active in that context,
and retains promoter/TSS ties as multiple rows rather than enforcing one
enhancer row.
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small,align=center},
  source/.style={draw=pipelineblue,rounded corners,thick,fill=lightgold,
    text width=6.1cm,minimum height=1cm,inner sep=6pt},
  branch/.style={draw=pipelineblue,rounded corners,thick,text width=3.75cm,
    minimum height=1.65cm,inner sep=6pt},
  product/.style={draw=gray!65,rounded corners,text width=3.75cm,
    minimum height=1.3cm,inner sep=5pt},
  flow/.style={->,thick,color=pipelineblue}
]
  \node[source] (input) {Posterior-qualified proximal/distal enhancer DHS\\+
    context-resolved annotated promoter nodes};
  \node[branch,fill=lightblue,below=13mm of input,xshift=-4.45cm] (observed) {
    Seven observed contexts\\ICE-balanced pixel $O$ and context expectation
    $E_c(d)$\\retain promoter edges with $O/E_c(d)\geq1$};
  \node[branch,fill=lightgreen,below=13mm of input] (nearest) {
    All nine contexts\\choose minimum $|\mathrm{summit}-\mathrm{TSS}|$ among
    active promoters\\no contact and no distance cutoff};
  \node[branch,fill=lightpurple,below=13mm of input,xshift=4.45cm] (powerlaw) {
    \texttt{e13} and \texttt{hid}\\atlas expectation
    $E_{\mathrm{atlas}}(d)$ within 1 Mb\\rank by $E_{\mathrm{atlas}}(d)P_p$};
  \node[product,below=8mm of observed] (observedout) {
    Active-contact gene candidates\\alternative promoters collapsed};
  \node[product,below=8mm of nearest] (nearestout) {
    Nearest-active-TSS candidates\\exact promoter/TSS ties retained};
  \node[product,below=8mm of powerlaw] (powerlawout) {
    Distance-model gene candidates\\observed and $O/E$ fields blank};

  \draw[flow] (input) -- (observed);
  \draw[flow] (input) -- (nearest);
  \draw[flow] (input) -- (powerlaw);
  \draw[flow] (observed) -- (observedout);
  \draw[flow] (nearest) -- (nearestout);
  \draw[flow] (powerlaw) -- (powerlawout);
\end{tikzpicture}
\caption{Three supplementary reports within the optional contact branch.
Observed contacts are measured context evidence, the nearest-active-TSS table
uses no contact values, and the power-law table is a distance-based ranking
rather than an imputed contact.  The unfiltered, one-row-per-enhancer default
nearest-annotated-TSS product is shown in
Figure~\ref{fig:catalog-link-branches}.}
\label{fig:target-evidence-branches}
\end{figure}
...
active-contact and nearest-active-promoter tables and, for distance-only
contexts, the active-distance table below
...
catalog browser assets.  It always reports the default nearest-TSS enhancer
count, tie count, and per-context number whose nearest promoter is active.
When contacts are configured, it additionally reports the
observed-versus-modeled contexts, active promoter counts, and edge/candidate
counts, including the supplementary nearest-active-TSS projection.  Its
JSON sidecar records every declared source and current output
...
\filep{*.active_contact_enhancer_gene_candidates.tsv.gz},
\filep{*.nearest_active_promoter_gene_candidates.tsv.gz}, and the
...
As written, this creates the default nearest-TSS branch and performs no
contact download.  Adding \texttt{--with-contacts} to this new configuration
also creates the canonical dm6 contact branch.  The choice is part of the
frozen scientific configuration and cannot be switched on during an in-place
checkpoint resume.  To add contacts after catalog completion, export a catalog
bundle and use a new run ID as below.
...
endpoint.  Choosing \stage{links} or \stage{report} builds the default
nearest-TSS table.  For the canonical nine-context dm6 atlas,
\texttt{--with-contacts} additionally downloads and normalizes the versioned
contact sources.  To
...
Contact promoter window & 1,000 bp ($\pm500$) & Centered on each distinct dm6 GTF gene/TSS node \\
...
Promoter active posterior & 0.5 & Requires a context-member DHS summit within 500 bp and H3K27ac high-component posterior \\
Default nearest-TSS enhancer posterior & none & Every proximal/distal enhancer receives one row; activity remains a context annotation \\
Optional focused enhancer posterior & 0.5 & Element-level threshold for observed-contact, nearest-active-TSS, and power-law projections \\
Focused observed/expected contact & 1.0 & Applied to observed promoter edges before gene collapse \\
Nearest active promoter & Chromosome-wide & Minimum summit-to-active-TSS distance; exact ties retained, no contact evidence \\
