...
\date{Repository documentation revision: 2026-07-28}
...
\texttt{control\_library}, peak caller, MACS3 or HMMRATAC parameters, adapters,
...
The blacklist is used during final BAM filtering and in the HMMRATAC branch.
The TSS BED supports ATAC TSS enrichment and regulatory-class annotation.  The
...
geometry, exponent range, component lengths, merge gap, or caller differs.
...
\subsection{Optional HMMRATAC branch}

HMMRATAC is an explicit alternative for paired-end ATAC only.  Defaults are a
lower cutoff of 10, upper cutoff of 20, and prescan cutoff of 1.2; the reference
blacklist is supplied to the caller.  Per-replicate and pooled-context
HMMRATAC peaks then enter the same replicate-support step used by the qpois
branch.  A context cannot mix HMMRATAC and qpois parameters.
...
Accepted ATAC libraries are grouped by the sample-sheet context.  For the
qpois path, their insertion BEDs are concatenated and a pooled context call is
made and refined.  For HMMRATAC, accepted BAMs are pooled for the context call.
The pooled peaks are not accepted merely because they are present in the
aggregate signal.
