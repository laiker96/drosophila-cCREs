...
balancing pass is applied to the merged count matrix.  This order gives each
read pair one contribution before normalization and avoids averaging
independently balanced matrices with incompatible scales or masks.
...
count, matrix dimensions, the finite-weight fraction, convergence status, and
biological caveats.  Conversion and coarsening copies under
...
The three graph products per context are
\filep{contexts/<context>.nodes.tsv.gz},
\filep{<context>.element_promoter_edges.tsv.gz}, and
\filep{<context>.element_gene_candidates.tsv.gz} below
...
\filep{*.element_gene_candidates.tsv.gz}, metrics, and provenance JSON \\
Integrated report &
...
Cooler & 0.10.4 & HiCExplorer & 3.7.6 \\
\bottomrule
\end{tabular}
\end{center}
